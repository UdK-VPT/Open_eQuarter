...
class LayerInteraction_test(unittest.TestCase):
    ...
    def test_add_layer_to_registry(self):
        ...
        self.assertEqual(len(QgsProject.instance().mapLayers()), number_of_layers, 'An error occured when trying to add a none-type-layer to the MapLayerRegistry. The number of layers should not increase.')
    ...
    def test_find_layer_by_name(self):
        ...
        for key, layer_value in reg.mapLayers().items():
