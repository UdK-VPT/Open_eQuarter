...
# differentiation between RB and NRB (for now in case of contemporary U-Values RB=NRB. After getting NRB data for contemporary case code must be adaptet)
    if parameters['BLD_USAGE'] == "RB":
        if parameters['HERIT_STAT'] == "0":
            ...
            if not oeq_global.isnull(parameters['YOC']):
                rf_ueh = rb_contemporary_roof_uvalue_by_building_age_lookup.get(parameters['YOC'])
        ...
        if parameters['HERIT_STAT'] == "0":
            ...
            if not oeq_global.isnull(parameters['YOC']):
                rf_ueh = nrb_contemporary_roof_uvalue_by_building_age_lookup.get(parameters['YOC'])
        ...
        if parameters['HERIT_STAT'] == "0":
            ...
            if not oeq_global.isnull(parameters['YOC']):
                rf_ueh = (((rb_contemporary_roof_uvalue_by_building_age_lookup.get(parameters['YOC'])) + (nrb_contemporary_roof_uvalue_by_building_age_lookup.get(parameters['YOC']))) / 2)
