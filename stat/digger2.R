...
setMethodS3("R_code", "OeQ_Inv", function(this,fun_name,description,filename=NULL,weights=NULL,...){
  ...
  l.code=substr(l.code, 1, nchar(l.code)-3)
  l.code=paste(l.code,")\n}\n")
  if (!is.null(filename)) writeLines(l.code,paste(filename,".R",sep=""))
  return(l.code)
})

# PYCODE MUSS ANGEPASST WERDEN
# generate py_code snippet for the bestfit model of this regression
setMethodS3("py_code", "OeQ_Inv", function(this,fun_name,description,filename=NULL,...){
  l.code=paste("# OeQ autogenerated correlation for '",description,"'\n\n",
               "import math\n",
               "import numpy as np\n",
               "def ",fun_name,"(xin,mode='distribution',weights=None):\n",
               "    if weights is None:\n",
               "        weights=np.repeat(1,",length(this$.regressions),")\n\n",sep="")
   l.columns=c()
   for (i in this$.regressions) {
  ...
  l.cnt=1
  for (i in l.columns)  {
    l.code=paste(l.code,i,"/l_sum * weights[",l.cnt-1,"] + ",sep="")
    l.cnt=l.cnt+1
  }
  l.code=substr(l.code, 1, nchar(l.code)-3)
  l.code=paste(l.code,")\n\n")
