    ...
    def register_information(self):
        ...
        if extension_name != self.placeholder:
            ...
            extension = extensions.OeQExtension(
                ...
                source_type=type,
                layer_name=layer_name,
                description='Generic extension for ' + layer_name,
                source=source_path,
                active=self.stateBox.isChecked())
...
class ColorPicker_dialog(QtWidgets.QDialog, Ui_color_picker_dialog):
    ...
    def remove_entry(self, model_index):
        ...
        if column == 4:
            ...
            color_key = list(model.in_data.keys())[row]
...
class ProjectSettings_form(QtWidgets.QDialog, Ui_project_settings_form):
    ...
    def update_form(self, index):
        ...
        if self.found_locations != []:
            ...
            if not oeq_global.isEmpty(location_info['house_number']):
                self.location_street.setText(self.location_street.text() + ' ' + location_info['house_number'])
    ...
    def get_location_from_web(self,index):
         ...
         if len(self.found_locations) == 1:
            ...
            self.location_city.addItem(_fromUtf8('< Select Location >'))
         ...
         if oeq_global.isEmpty(self.location_city.text()) & oeq_global.isEmpty(self.location_street.text()) & (not oeq_global.isEmpty(self.location_postal.text())):
            ...
            if len(self.municipals) > 0:
                if len(self.municipals) ==  1:
                    ...
                    self.combobox_city_layout()
                    self.location_city.addItem(_fromUtf8('< Select Location >'))
                    for i in [ str(i['POSTCODE']) + ' ' + i['NAME'] for i in self.municipals]:
            ...
            self.municipals = [x for x in self.municipal_information.municipal_db if x['NAME'].split()[0].split(',')[0] == city.split()[0].split(',')[0]]
            if len(self.municipals) > 0:
                if len(self.municipals) ==  1:
                    self.lineedit_city_layout()
                    self.location_city.setText(self.municipals[0]['NAME'])
    ...
    def location_by_coordinates(self):
        ...
        if len(self.found_locations) == 1:
            ...
            self.combobox_city_layout()
            self.location_city.clear()
            self.location_city.addItem(_fromUtf8('< Select Project Location >'))
            for loc in self.found_locations:
                self.location_city.addItem(_fromUtf8(loc['formatted_location']))
            self.location_city.currentIndexChanged.connect(self.update_form)
    ...
    def find_municipal_information(self):
        ...
        if ((city == '') | (city == '')):
            ...
            if ((city != '') & (city != '')):
                self.municipals = [x for x in self.municipal_information.municipal_db if ((x['NAME'] == city) | x['NAME'].startswith(city+',') | x['NAME'].startswith(city+' '))]
                if len(self.municipals) > 0:
                    if len(self.municipals) ==  1:
                        self.lineedit_postal_layout()
                        self.location_postal.setText(str(self.municipals[0]['POSTCODE']))
...
class EstimatedEnergyDemand_dialog(QtWidgets.QDialog, Ui_EstimatedEnergyDemand_dialog):
    def __init__(self):
        ...
        vector_layers = [x for x in list(layer_dict.values()) if x.type() == QgsMapLayer.VectorLayer]
