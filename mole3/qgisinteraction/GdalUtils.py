...
def getGdalBinPath():
    ...
    return settings.value(base_path, "", type=str)
...
def setMacOSXDefaultEnvironment():
    ...
    qgis_bin = "%s/bin" % QgsApplication.prefixPath()   # path to QGis bin folder
    qgis_python = "%s/Resources/python" % qgis_app    # path to QGis python folder
    ...
    gdal_base_path = "/Library/Frameworks/GDAL.framework/Versions/%s.%s" % (gdal_versionsplit[0], gdal_versionsplit[1])
    ...
    if os.path.exists( qgis_standalone_gdal_path ):  # qgis standalone
        ...
        if getHelpPath() == '':
            setHelpPath( "%s/Resources/doc" % qgis_standalone_gdal_path )

    elif os.path.exists( gdal_base_path ):
        # all GDAL parts are in the GDAL framework folder
        if getGdalBinPath() == '':
            setGdalBinPath( "%s/Programs" % gdal_base_path )
        if getGdalPymodPath() == '':
            setGdalPymodPath( "%s/Python/%s.%s/site-packages" % (gdal_base_path, sys.version_info[0], sys.version_info[1]) )
        if getHelpPath() == '':
            setHelpPath( "%s/Resources/doc" % gdal_base_path )