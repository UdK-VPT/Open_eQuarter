define([
    ...
    'js/oeq-utils',
    'js/oeq-layermenu'
], function($, ol, dr, Utils, LayerList){
    ...
    var shapes = Utils.layerFromGeoJSON('layers/BLD_Shapes.geojson', STYLE, STYLE_CACHE);
    ...
    MAP.addLayer(shapes);
    LayerList.update(MAP);
    MAP.getLayers().on('change', function(){LayerList.update(MAP)});
    MAP.getLayers().on('add', function(){LayerList.update(MAP)});
    MAP.getLayers().on('remove', function(){LayerList.update(MAP)});
});