...
# generate R_code snippet for this model
setMethodS3("R_code", "OeQ_Model", function(this,par_name,description=NULL,...){
...
setMethodS3("py_code", "OeQ_Model", function(this,par_name,description=NULL,...){
  if(is.null(description)) description=par_name
  l.coeff=this$.model$coefficients
  l.coeff[is.na(l.coeff)]=0
  l.code=paste("    # OeQ autogenerated correlation for '",description,"'\n",
...
# generate R_code snippet for the bestfit model of this regression
setMethodS3("R_code", "OeQ_Regr", function(this,par_name=NULL,...){
  if(is.null(par_name)) par_name = this$.columnname
  l.coeff=this[[this$.bestfit]]$.model$coefficients
  l.coeff[is.na(l.coeff)]=0
  return(this[[this$.bestfit]]$R_code(par_name,VERBOSE[par_name,]$description))
})


# generate py_code snippet for the bestfit model of this regression
setMethodS3("py_code", "OeQ_Regr", function(this,par_name=NULL,...){
  if(is.null(par_name)) par_name = this$.columnname
  l.coeff=this[[this$.bestfit]]$.model$coefficients
  l.coeff[is.na(l.coeff)]=0
  return(this[[this$.bestfit]]$py_code(par_name,VERBOSE[par_name,]$description))
})
...
# generate R_code snippet for the bestfit model of this regression
setMethodS3("R_code", "OeQ_Inv", function(this,fun_name,description,filename=NULL,weights=NULL,...){
  l.code=paste("# OeQ autogenerated correlation function: ",description,"\n",
               fun_name,"<-function(xin,mode='distribution',weights=NULL,...){\n",sep="")
  l.columns=c()
  if (is.null(weights)) weights=rep(1,length(this$.regressions))
      for (i in this$.regressions) {
        l.code=paste(l.code, "x=xin\n\n",sep="")
    l.code=paste(l.code, i$R_code(),sep="") 
...
# generate py_code snippet for the bestfit model of this regression
setMethodS3("py_code", "OeQ_Inv", function(this,fun_name,description,filename=NULL,weights=NULL,...){
  if (is.null(weights)) weights=rep(1,length(this$.regressions))
  l.code=paste("# OeQ autogenerated correlation for '",description,"'\n\n",
               "import math\n",
               "import numpy as np\n",
   ...
   l.columns=c()
   for (i in this$.regressions) {
    l.code=paste(l.code, i$py_code(),sep="") 
    l.columns=c(l.columns,i$.columnname)
  } 
  l.code=paste(l.code,"\n    l_sum = ")
  for (i in l.columns)  l.code=paste(l.code,i," + ",sep="")
...
setMethodS3("distribution_plot", "OeQ_Inv", function(this,
  ...
  par(xpd=TRUE)
  if(!is.null(pdffile)) {
    pdf(paste(PDF_PATH,pdffile,sep="/"),paper="a4", width = 0, height = 0)
    par(mfrow=c(2,1))
  ...
  if(!is.null(pdffile)) {dev.off()
                         browseURL(paste(PDF_PATH,pdffile,sep="/"))}
})
...
build_type_distribution_by_population_density<-function(
  ...
  l.investigation$R_code(fun_name=name,filename=name,description=description,weights=BUILDINGS_BY_NOFLATS_WEIGHTS)
  l.investigation$py_code(fun_name=name,filename=name,description=description,weights=BUILDINGS_BY_NOFLATS_WEIGHTS)
}
...
build_age_distribution_by_average_year_of_construction<-function(
  ...
  description="Buildings built in correlation to year of construction"){
  l.investigation=new_OeQ_Inv(BLD_DB[,c("BLD_AGE1_AVG",BUILDINGS_BY_AGE1)],normcolumn="BLD_AGE1_TOTAL",p_mode="log")
  l.investigation$distribution_plot(pdffile=name)
  l.investigation$sum_plot(pdffile=paste(name,"_sum",sep=""))
  l.investigation$R_code(fun_name=name,filename=name,description=description)
  l.investigation$py_code(fun_name=name,filename=name,description=description)
}
