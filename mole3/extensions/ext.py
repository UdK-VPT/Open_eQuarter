...
def average(self=None, parameters={}):
 ...
 #   print parameters
    ...
    if parameters != {}:
        ...
        try:
            result[self.field_id + '_M']['value'] = sum([float(i) for i in list(parameters.values())]) / len(parameters)
        except:
            pass
...
class OeQExtension:
    ...
    def evaluateAll(self, parameter={},feature = None):
       ...
       # save visibility states and st them to 'visible'
        ...
        progressbar = oeq_global.OeQ_push_progressbar('Extension "' + self.extension_name + '":',
                                                      'Updating layer "' + self.targetlayer_name + '" from "' + self.sourcelayer_name + '"!',
        ...
        for srcFeat in source_layer.getFeatures():
            ...
            if target_layer == source_layer:
                ...
                tgtFeat = [x for x in target_layer.getFeatures() if x[config.building_id_key] == srcFeat[config.building_id_key]]
    ...
    def load_wfs(self,extent=None,forceload = False):
        ...
        #init progressbar
        progressbar = oeq_global.OeQ_push_progressbar('Extension "' + self.extension_name + '":',
                                                              'Be patient! Loading WFS-Map "' + self.layer_name + '" may take long!',
                                                              maxcount=7)
        progress_counter = oeq_global.OeQ_update_progressbar(progressbar, 0)
        ...
        if not wfsLayer.isValid():
            oeq_global.OeQ_push_error('Extension "' + self.extension_name + '":',
                                      'Could not load WFS-Map "' + self.source + '"!')
            return None
        ...
        if not oeq_global.OeQ_wait_for_renderer(60000):
            ...
            return False
        if not wfsLayer.isValid():
            oeq_global.OeQ_push_error('Extension "' + self.extension_name + '":', 'Could not add WFS-Map "' + self.layer_name + ' to Legend"!')
            return None
...
file_writer.writeRaster(pipe,
    ...
    def decode_color_table(self):
        ...
        if self.source_type == 'wms':
            if self.colortable != None:
                ...
                progressbar = oeq_global.OeQ_push_progressbar('Extension "' + self.extension_name + '":',
                                                              'Decoding colors in "' + self.sourcelayer_name + '"!',
    ...
    def required(self):
        ...
        if (self.sourcelayer_name != self.layer_name): #(self.sourcelayer_name != config.data_layer_name) &
            sourcelayer_supplier = [ext for ext in all_suppliers if (self.sourcelayer_name == ext.layer_name) & bool(self.sourcelayer_name)]
        ...
        if (self.targetlayer_name != self.layer_name): #(self.targetlayer_name != config.data_layer_name) &
            targetlayer_supplier = [ext for ext in all_suppliers if (self.targetlayer_name == ext.layer_name) & bool(self.targetlayer_name)]
        ...
        if bool(self.par_in):
            for ipar in self.par_in:
                parameter_supplier = [ext for ext in all_suppliers if ipar in ext.get_par_out()]
    ...
    def work_out_results(self):
        ...
        result = dict(list(zip([x[config.building_id_key] for x in database.getFeatures()],[x[self.show_results[0]] for x in database.getFeatures()])))
...
def by_category(category=None, registry=None):
    ...
    if category == None:
        return registry
    return [ext for ext in registry if ext.category == category]
...
def by_subcategory(subcategory=None, registry=None):
    ...
    if subcategory == None:
        return registry
    return [ext for ext in registry if ext.subcategory == subcategory]
...
def by_state(active=None, category=None, registry=None):
    ...
    if active == None:
        return registry
    return [ext for ext in registry if ext.active == active]
...
def by_name(name=None, category=None, active=None, registry=None):
    ...
    if name == None:
        return registry
    return [ext for ext in registry if ext.extension_name == name]
...
def by_layername(name=None, category=None, active=None, registry=None):
    ...
    if name == None:
        return registry
    return [ext for ext in registry if ext.layer_name == name]
...
def by_type(type=None, category=None, active=None, registry=None):
    ...
    if type == None:
        return registry
    return [ext for ext in registry if ext.source_type == type]
...
def by_field_id(field_id=None, category=None, active=None, registry=None):
    ...
    if field_id == None:
        return registry
    return [ext for ext in registry if ext.field_id == field_id]
...
def by_layerid(layer_id=None, category=None, active=None, registry=None):
    ...
    if layer_id == None:
        return registry
    return [ext for ext in registry if ext.layer_id == layer_id]
...
def by_targetlayer_name(targetlayer_name=None, category=None, active=None, registry=None):
    ...
    if targetlayer_name == None:
        return registry
    return [ext for ext in registry if ext.targetlayer_name == targetlayer_name]
...
def by_sourcelayer_name(sourcelayer_name=None, category=None, active=None, registry=None):
    ...
    if sourcelayer_name == None:
        return registry
    return [ext for ext in registry if ext.sourcelayer_name == sourcelayer_name]
...
def by_extension_id(extension_id=None, category=None, active=None, registry=None):
    ...
    if extension_id == None:
        return registry
    return [ext for ext in registry if ext.extension_id == extension_id]
...
def by_extension_type(extension_type=None, category=None, active=None, registry=None):
    ...
    if extension_type == None:
        return registry
    return [ext for ext in registry if ext.extension_type == extension_type]
